...
import { getCompanyName, hasId, toAbsoluteUrl } from "../utils/domain-utils.ts";

test("hasId - numeric IDs", () => {
  assert.strictEqual(hasId("https://careers.chime.com/en/jobs/8322458002"), true);
  assert.strictEqual(hasId("https://careers.chime.com/en/jobs/09bf6010-898c-42c0-b9f8-580aa058d75c"), true);
...
test("hasId - GUID-like IDs", () => {
  assert.strictEqual(hasId("https://jobs.lever.co/company/cee89c66-dd27-4242-9cbc-d0316cb3e3a6"), true);
  assert.strictEqual(hasId("https://example.com/jobs/abcd-1234-ef30-5678"), true);
  ...
  assert.strictEqual(hasId("https://doctolib.legal/B2C-Cookie-Policy-Career-COM"), false);
});

test("hasId - mixed real-world cases", () => {
  assert.strictEqual(hasId("https://careers.chime.com/en/jobs/8322458002/senior-software-engineer"), true);
  assert.strictEqual(hasId("https://jobs.lever.co/kraken/cee89c66-dd27-4242-9cbc-d0316cb3e3a6/apply"), true);
  ...
  assert.strictEqual(hasId("https://www.zenrows.com/careers/job?gh_jid=4772531101"), true);
  assert.strictEqual(hasId("https://job-boards.greenhouse.io/wrike/jobs/4676119005"), true);
});
...
test("getCompanyName - with paths", () => {
  assert.strictEqual(getCompanyName("https://duolingo.com/careers"), "duolingo");
  assert.strictEqual(getCompanyName("https://doctolib.fr/jobs/123"), "doctolib");
});
...
test("toAbsoluteUrl", () => {
  ...
  assert.strictEqual(toAbsoluteUrl({ url: base, href: "https://google.com/jobs" }), "https://google.com/jobs");
  assert.strictEqual(toAbsoluteUrl({ url: base, href: "/careers" }), "https://example.com/careers");
  assert.strictEqual(toAbsoluteUrl({ url: base, href: "jobs" }), "https://example.com/path/jobs");
  assert.strictEqual(toAbsoluteUrl({ url: base, href: "#section" }), "https://example.com/path/page#section");
});
