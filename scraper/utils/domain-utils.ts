...
export const toAbsoluteUrl = ({ url, href }: { url: string; href: string }): string => {
  if (!href) {
    return url;
  }
...
export const extractCompanyName = ({ url, regexes }: { url: string; regexes: RegExp[] }): string => {
  const companyName = regexes.map((r) => url.match(r)?.[1]).find(Boolean) ?? null;
  if (!companyName) {
    throw Error(`[${extractCompanyName.name}]: URL ${url} does not match expected pattern`);
  }
  return companyName;
};

export const getUrlWithParams = ({ url, params }: { url: string; params?: Record<string, any> }): string => {
  if (!/^https:\/\//i.test(url)) {
    return "";
  }
