...
import { getCompanyNamesGeneratorPrompt } from "./prompts/company-name-generator-prompt";
import { COMPANIES_DATA_FILES } from "./companies-utils";

export const generateCompaniesNames = async ({
  count,
  location,
}: {
  count: number;
  ...
  const oldCompanies = readJSONFile<CompanyInfo[]>(
  ...
  const allCompanies = [
    ...
    ...newCompanies
      .map((name) => ({ name }))
      .filter(({ name }) => !oldCompaniesSet.has(name)),
  ];
  ...
  console.log(
    `Added after merge: ${allCompanies.length - oldCompanies.length}`,
  );
